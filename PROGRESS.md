...
- **PLAN acceptance contract:** neutral scene renders with correct aspect at every matrix viewport; actual drawing buffer obeys selected pixel cap; hidden/resumed tab has no large delta; repeated create/dispose returns resource counts to baseline; camera begins upright with fixed defaults.
  - **Acceptance evidence (every clause):** Neutral scene/aspect: the corrected focused `npm run test:browser -- viewport.spec.ts` run passed exactly 14/14 in 7.2s across desktop Chromium 1280×720 and mobile Chromium 390×844; `tests/browser/viewport.spec.ts:110-124` covers the 320×568 and 1920×1080 matrix cases and `:127-143` covers portrait→desktop resize without runtime recreation. Every matrix, DPR, and resize assertion now compares the reported viewport buffer with the actual `HTMLCanvasElement.width/height` via `tests/browser/viewport.spec.ts:82-101,118-123,136-143,145-163`, while retaining camera-aspect and cap checks. The earlier combined 46/46 total is historical context only (14 viewport cases plus 32 lifecycle cases), not the focused C03 result. Durable manual neutral capture remains `local://c03-evidence/c03-neutral-1280x720.png`, with canvas CSS/buffer metrics 1246×552 and camera aspect 2.2572. Drawing-buffer cap: the 1920×1080 emulated-DPR-3 measurement was CSS 1886×892, effective DPR 1.5611, actual buffer 2944×1392 = 4,098,048 pixels, within the 4,100,000 cap. Hidden/resumed timing: `tests/browser/viewport.spec.ts:176-258` proves the loop stops while hidden, the frame count stays frozen for 250ms, and the first resumed delta is ≤0.001s and not 0.1s, followed by a positive cadence below 0.1s. Repeated teardown/rebuild: `tests/browser/viewport.spec.ts:260-283` proves ten dispose/create cycles leave one live runtime and stable resource counts; the 1280×720 manual metrics reported resources 2, references 2, groups 1. Upright camera defaults: `tests/browser/viewport.spec.ts:165-174` and the manual metrics report fov 50, near 0.1, far 100, position [0,1.5,5], and up [0,1,0].
- **PLAN focused verification contract:** `npm run test:unit -- frameClock`; `npm run test:browser -- viewport.spec.ts`; manually resize 320×568 → 1920×1080 at DPR 1 and emulated DPR 3; hide/resume tab; run ten runtime create/dispose cycles through a test hook.
  - **Focused-verification evidence (every command/scenario):** Parent `npm run test:unit -- frameClock` passed 1 file/9 tests (`tests/unit/frameClock.test.ts:9-99`), including the fresh-baseline-after-pause and idempotent-disposal cases. Parent `npm run test:unit -- resourceRegistry` passed 1 file/4 tests (`tests/unit/resourceRegistry.test.ts:9-104`), including shared final-owner disposal, rebuild-group release, idempotent dispose-all, and cleanup after a throwing disposer. The new `npm run test:unit -- threeRuntime` passed 1 file/6 tests (`tests/unit/threeRuntime.test.ts:73-208`), covering late initialization after listener/loop installation with complete unwind, failed rebuild builder rollback, failed rebuild commit rollback, throwing resource disposal that still finalizes renderer/marker/counters, healthy disposal idempotence, and actual `installPageHideHandler` persisted/nonpersisted `EventTarget` wiring. Parent `npm run test:browser -- viewport.spec.ts` selected only the focused viewport suite and passed exactly 14/14 in 7.2s; its matrix/DPR/resize cases compare actual canvas buffers at every tested state, and its visibility case proves a first resumed delta ≤0.001s, not 0.1s, followed by a positive value below 0.1s. The separate regression `npm run test:browser -- lifecycle.spec.ts` passed exactly 32/32 in 14.1s; it is not included in the focused 14/14 count. Manual resize 320×568 → 1920×1080 at DPR 1 and emulated DPR 3, hide/resume, and the development ten-cycle create/dispose hook all passed with the metrics and durable capture recorded above. Parent `npm run typecheck` passed at `artifact://415`. Parent `npm run build` passed at `artifact://416`; the only noted issue was the nonfatal single-chunk warning above 500 kB. Production `dist` inspection found no `three-runtime:command`, `data-three-runtime-metrics`, or runtime-cycle/rebuild markers.
- **PLAN review surface:** ownership/disposal, resize arithmetic, color-space settings, camera clipping, animation-loop and visibility behavior.
  - **Implementation-review evidence:** Round 1 packets for draft commit `236b001`: `agent://C03RuntimeReviewer` (RUNTIME-001, RUNTIME-002), `agent://C03LifecycleReviewer` (LIFE-001, LIFE-002), `agent://C03TestReviewer` (TEST-001, TEST-002, TEST-003), and `agent://C03AccountingReviewer` (ACC-001, ACC-002, ACC-003). Parent fix verification now records implemented fixes and proof for all ten findings in the acceptance/focused fields and Round-1 record below: BFCache-safe `pagehide` handling, transactional late-init and rebuild rollback, best-effort disposal/finalization, fresh-resume assertions, fail-closed focused browser scope, actual-buffer matrix/resize assertions, exact unit/browser counts, typecheck/build artifacts, and production marker absence. No independent re-review has been performed, no clean claim is made, the Round-1 result remains finding-bearing, and fix evidence plus review-fix commit `fe7e3b6` are recorded; clean re-review remains pending. Prior C03 acceptance/focused evidence and review history remain retained above.
- **Planned commit boundary:** `feat: establish the Three.js rendering lifecycle`
- **Implementation commit hash(es):** `236b001` (draft implementation commit), `fe7e3b6` (review-fix commit).
- **Append-only implementation review and fix/re-review rounds:**

| Round | Finding IDs | Review/fix evidence | Commit | Reviewer/date | Result (`clean` or new finding IDs) |
|---|---|---|---|---|---|
| 1 | RUNTIME-001, RUNTIME-002, LIFE-001, LIFE-002, TEST-001, TEST-002, TEST-003, ACC-001, ACC-002, ACC-003 | Draft-commit Round 1 review packets: `agent://C03RuntimeReviewer`, `agent://C03LifecycleReviewer`, `agent://C03TestReviewer`, and `agent://C03AccountingReviewer`; parent fix verification: RUNTIME-001 — `src/app/AppController.ts:51-58,99-101` forwards the real `PageTransitionEvent` and skips destruction for persisted BFCache pagehide, while `tests/unit/threeRuntime.test.ts:188-208` exercises persisted and nonpersisted events through the actual `installPageHideHandler` EventTarget wiring. RUNTIME-002/LIFE-002 — `src/render/ThreeRuntime.ts:154-208,242-317` unwinds late initialization and transactionally rolls back failed scene builds and rebuild commits; `tests/unit/threeRuntime.test.ts:73-148` verifies listener/loop/viewport/resource/renderer unwind and live-scene/ownership preservation. LIFE-001 — `src/render/ThreeRuntime.ts:288-353` runs best-effort teardown and finalization; `tests/unit/threeRuntime.test.ts:150-186` verifies renderer disposal, loop stop, marker removal, disposed counters, and idempotence even when resource cleanup throws; `tests/unit/resourceRegistry.test.ts:64-104` verifies idempotent and throwing-disposer accounting. TEST-001 — `tests/browser/viewport.spec.ts:176-258` rejects the 0.1 clamp as the first resumed sample and requires a later positive cadence below 0.1s. TEST-002/ACC-001 — the focused `npm run test:browser -- viewport.spec.ts` run is exactly 14/14 in 7.2s, with the separate lifecycle regression exactly 32/32 in 14.1s. TEST-003/ACC-003 — `tests/browser/viewport.spec.ts:82-101,118-123,136-143,145-163` compares actual canvas buffers at every matrix/DPR/resize state and applies the pixel cap. ACC-002 — the acceptance/focused ledger now records the fresh-baseline result, exact command scopes/counts, and parent proof without conflating the historical 46/46 combined total. Typecheck/build and production-marker proof are recorded above; prior evidence/history is retained. Fix evidence and review-fix commit `fe7e3b6` are recorded; clean re-review remains pending and no clean claim is made. | `fe7e3b6` | C03RuntimeReviewer; C03LifecycleReviewer; C03TestReviewer; C03AccountingReviewer / 2026-07-12 | RUNTIME-001, RUNTIME-002, LIFE-001, LIFE-002, TEST-001, TEST-002, TEST-003, ACC-001, ACC-002, ACC-003 |
