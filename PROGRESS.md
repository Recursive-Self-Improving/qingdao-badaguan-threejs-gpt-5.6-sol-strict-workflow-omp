...
- [x] Gate startup on WebGL2 capability and render a useful non-WebGL fallback without constructing the Three.js runtime.
  - **Implementation evidence:** `src/platform/capabilities.ts:1-62` provides the typed WebGL2 probe/result seam, fail-closed exception handling, and best-effort context cleanup; `src/app/AppController.ts:35-44,108-119` gates capability evaluation and exposes `lifecycle=unsupported` only in DEV; `src/main.ts:3-14` exposes the typed `capability=supported` override only in DEV; `tests/browser/lifecycle.spec.ts:139-153` verifies the forced negative path remains `unsupported` through Retry with zero runtime markers before and after; final production `dist` inspection after `artifact://165` found no `capability=supported`, `lifecycle=`, `data-three-runtime`, or `data-renderer` seam/marker strings.
