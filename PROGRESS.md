...
- [x] Implement the explicit application state machine and event-driven transitions for boot, loading, onboarding, exploring, paused, degraded, context-lost, unsupported, and fatal states.
  - **Implementation evidence:** `src/app/appState.ts:1-385` defines the typed nine-state `AppState` union, complete `AppEvent` set, visible-output/action invariants, safe `accepted`/`transitioned` results, terminal precedence, degraded underlying projections, panel transitions, and deterministic retry; `src/app/AppController.ts:1-177` owns event-driven BOOT, capability, Retry, forced-scenario, Escape, and UI-action dispatch; `tests/unit/appState.test.ts:16-263` exercises every state/event matrix plus nonblank projections, readiness/start gating, lock/fallback, pause/resume, precedence, degraded mode, retry, illegal/duplicate input, exact copy, and panels. Parent `npm run test:unit -- appState` passed 1 file/10 tests; parent `npm run typecheck` passed at `artifact://162` and final `npm run build` passed at `artifact://165`.
- **Round-1 fix verification (STATE-001, TEST-003):** Parent reran `npm run test:unit -- appState` with 1 file/10 tests passing. The exhaustive transition oracle iterates every enriched state in `TRANSITION_STATES` against every event in `ALL_EVENTS`, asserting exact next state, `accepted`, `transitioned`, unchanged object identity for rejected events, and invariant equality; the explicit cases also cover locked Escape pause/resume, unlock-during-pause, fatal/context-loss precedence, degraded projections, deterministic Retry, illegal/duplicate events, exact copy, and panels. Parent `npm run typecheck` passed at `artifact://202`; parent `npm run build` passed at `artifact://203`.
- [x] Implement the DOM overlay system, live status region, focus management, onboarding/help/settings panels, and deterministic Start/Retry/Resume actions; onboarding identifies Badaguan and visibly explains WASD/arrows, mouse or drag/touch look, and Escape, while locked, unlocked, and pointer-lock-fallback modes have concise visible status.
  - **Implementation evidence:** `index.html:18-99` supplies the semantic overlay shell, actions/panels/controls regions, canvas description, and sole `#app-status`; `src/app/config.ts:1-23` owns the exact onboarding, locked, fallback, paused, retry, and failure copy; `src/ui/AppUI.ts:1-890` implements state projections, Start/Pause/Resume/Retry actions, Help/Settings fragments, keyboard Escape handling, one polite atomic status channel, transition-key announcement deduplication, ready-only Start focus, panel-heading focus, and trigger focus return; `src/app/AppController.ts:55-105` routes Escape and UI actions through the reducer. `src/ui/AppUI.ts:185-232,307-445,448-623,628-884` specifically covers fallback wording, every projection/action, panel content, and rendering/focus/status behavior.
...
- [x] Gate startup on WebGL2 capability and render a useful non-WebGL fallback without constructing the Three.js runtime.
  - **Implementation evidence:** `src/platform/capabilities.ts:1-62` provides the typed WebGL2 probe/result seam, fail-closed exception handling, and best-effort context cleanup; `src/app/AppController.ts:35-44,108-119` gates capability evaluation and exposes `lifecycle=unsupported` only in DEV; `src/main.ts:3-14` exposes the typed `capability=supported` override only in DEV; `tests/browser/lifecycle.spec.ts:198-206` verifies `/?capability=unsupported-then-supported`: the full-union marker selector `[data-three-runtime], canvas[data-renderer], [data-renderer-runtime], [data-runtime-created]` has count 0 before `Retry` in exact `unsupported` state and count 0 again after `Retry` reaches supported onboarding with focused `Start`; the latest `npm run test:browser -- lifecycle.spec.ts` rerun passed 32/32 in 7.0s across desktop Chromium 1280×720 and mobile Chromium 390×844; final production `dist` inspection after `artifact://165` found no `capability=supported`, `lifecycle=`, `data-three-runtime`, or `data-renderer` seam/marker strings.
- **Round-1 fix verification (TEST-001, ACC-004):** The typed DEV-only `unsupported-then-supported` seam starts in unsupported, Retry re-runs capability evaluation, and reaches supported onboarding. `tests/browser/lifecycle.spec.ts:198-206` asserts the full-union marker selector `[data-three-runtime], canvas[data-renderer], [data-renderer-runtime], [data-runtime-created]` has count 0 before `Retry` in exact `unsupported` state and count 0 again after `Retry` reaches supported onboarding with focused `Start`; the latest lifecycle rerun passed 32/32 in 7.0s across desktop Chromium 1280×720 and mobile Chromium 390×844. Final production `dist` inspection after `artifact://203` found no DEV seam query strings (`capability=supported`, `capability=unsupported-then-supported`, or `lifecycle=`) and no runtime marker strings (`data-three-runtime`, `data-renderer`, or `data-runtime-created`).
- **PLAN acceptance contract:** every state has a visible, non-blank projection; onboarding identifies Badaguan and visibly explains mouse look, drag/touch fallback, WASD/arrows, and Escape; confirmed lock shows “Press Escape to release”; unlocked and denied/error states visibly name drag + keyboard or touch exploration; Start works by click, Enter, and Space and is focused only when ready; illegal transitions fail safely; retry is deterministic; a forced WebGL2-negative path never constructs the renderer; status announcements are transition-based.
  - **Acceptance evidence (every clause):** (1) Every state has a visible nonblank projection: `src/app/appState.ts:81-171` maps boot, loading, onboarding, exploring, paused, degraded, context-lost, unsupported, and fatal to nonblank invariant output, `src/ui/AppUI.ts:307-415` renders each projection, and `tests/unit/appState.test.ts:49-57` checks every state and event result for trimmed nonblank output; the parent unit run passed 1 file/10 tests. (2) Onboarding identifies Badaguan and visibly explains mouse look, drag/touch fallback, WASD/arrows, and Escape: `src/app/config.ts:4-8` and `src/ui/AppUI.ts:333-344,463-482` provide the exact detail and guide, and parent manual browser inspection showed the exact onboarding copy at 1280×720 and 390×844. (3) Confirmed lock shows exactly `Press Escape to release`: `src/app/config.ts:6,19-22` and `src/ui/AppUI.ts:345-358` select that detail/status, and the forced locked browser projection plus manual inspection matched it. (4) Unlocked and denied/error projections visibly name drag plus keyboard or touch: `src/ui/AppUI.ts:185-195` supplies distinct denied/error/unlocked sentences, and parent manual inspection plus the forced denied/error/unlocked browser projections showed those fallback details/statuses. (5) Start works by click, Enter, and Space and is focused only when ready: `src/ui/AppUI.ts:424-444,846-865` emits Start only when `canStart` and focuses it on the first ready render; `tests/browser/lifecycle.spec.ts:27-53,87-100` covers ready focus, click Start, Enter, Space, and Resume, while parent manual inspection confirmed Start focus only in ready onboarding. (6) Illegal transitions fail safely: `src/app/appState.ts:174-183,185-385` returns the unchanged state with `accepted:false`/`transitioned:false` for rejected events, `tests/unit/appState.test.ts:208-221` proves illegal and duplicate input safety, and `tests/browser/lifecycle.spec.ts:102-114` proves Escape does not mutate onboarding or create Resume/Retry actions. (7) Retry is deterministic: `src/app/appState.ts:309-383` sends retryable context-lost/degraded/unsupported/fatal states to loading and `src/app/AppController.ts:93-119` reevaluates capabilities after accepted Retry; `tests/unit/appState.test.ts:192-205` and the unsupported browser before/after-Retry scenario prove the deterministic result. (8) A forced WebGL2-negative path never constructs the Three.js runtime: `src/app/AppController.ts:108-119` dispatches unsupported from the capability result before any runtime integration, `src/main.ts:3-14` keeps the supported override DEV-only, and `tests/browser/lifecycle.spec.ts:198-206` asserts `/?capability=unsupported-then-supported` with the full-union marker selector `[data-three-runtime], canvas[data-renderer], [data-renderer-runtime], [data-runtime-created]` at count 0 before `Retry` in exact `unsupported` state and count 0 again after `Retry` reaches supported onboarding with focused `Start`; the latest lifecycle rerun passed 32/32 in 7.0s across desktop Chromium 1280×720 and mobile Chromium 390×844. Production `dist` grep found no `capability=supported`, `lifecycle=`, `data-three-runtime`, or `data-renderer` strings. (9) Status announcements are transition-based: `src/ui/AppUI.ts:216-232,649-669,836-844` keys announcements by state/projection and changes the sole atomic polite status only for a new key; parent manual inspection and browser assertions found exactly one `[aria-live]`/`#app-status` region at both required viewports. (10) Separate manual browser checks, outside the automated 24/24 suite, covered the previously structural-only projections: after Start, degraded showed nonblank `REDUCED MODE · EXPLORING`, detail naming drag plus keyboard/touch exploration, Pause/Retry/Help/Settings actions, and a persistent reduced-mode status; screenshot `local://c02-evidence/c02-degraded-1280x720.png`. Context-lost showed nonblank graphics-recovery detail/status with Retry and Help; screenshot `local://c02-evidence/c02-context-lost-1280x720.png`. Fatal showed nonblank stopped-state detail/status with the forced reason, Retry and Help; screenshot `local://c02-evidence/c02-fatal-1280x720.png`. These are manual observations, not automated browser assertions.
...
- **Round-2 acceptance fix verification (TEST-005, UI-004, UI-005, ACC-005, ACC-006; prior acceptance evidence retained):** Settings automation in `tests/browser/lifecycle.spec.ts:151-175` opens Settings with Space, asserts the panel is visible and `aria-expanded="true"`, asserts both `#app-overlay` and `#app-controls` have `inert`, presses Tab and verifies focus is on Close, presses Shift+Tab and verifies the active element is outside both inert regions, then presses Escape and verifies the panel is hidden, focus returns to the Settings trigger, `aria-expanded="false"`, and both inert attributes are removed. The post-assertion lifecycle rerun passed 32/32 in 6.9s. With JavaScript disabled at 320×568, DOM checks found the canvas, `#app-overlay`, and `#app-status` `display:none`; one authoritative no-JS narrative remained visible with no horizontal/vertical overflow, captured at `local://c02-evidence/c02-nojs-320x568.png`. At 1280×720, DOM measurements for both unsupported and fatal notices placed each complete notice fully inside its panel; the panel had `overflowY:auto`, `clientHeight=482`, and `scrollHeight=493` as the vertical overflow affordance, while the page had no horizontal/vertical overflow. Refreshed captures are `local://c02-evidence/c02-unsupported-1280x720.png` and `local://c02-evidence/c02-fatal-1280x720.png`. Parent static checks passed with typecheck `artifact://268` and build `artifact://269`. These findings are now concretely evidenced; Round-2 fix evidence and commit `2dca825` are recorded, while an independent terminal-clean re-review remains pending.
